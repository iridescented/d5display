...
float count_sustain(void) {//count the sustainable energy
	float a, b;
	a = ((float)adc_pv()/ MAXADC)*5; //varying from 0 to 5V
	b = ((float)adc_wind()/ MAXADC)*5;//varying from 0 to 5V
	sustain = a + b;
	return sustain;
}
float count_power(void) {
...
int main(void)
{
	...
	for (;;) {
		...
		mux(2);
		g = read_adc(); //bus current
		l = ((double)g *3.3) / MAXADC;
		// printf("ADC value = %d , BusCurrent = %d \n", g, (int)l);
	

		mux(3);
		h = read_adc(); //bus voltage
		m = ((double)h *3.3) / MAXADC;
	...
	while (1) {
		adc_pv();
		// Sources[2] = ((float)pv/ MAXADC)*5;
		adc_wind();
		// Sources[1] = ((float)wind/ MAXADC)*5;
		adc_bc();
		// BasicInfo[2] = ((float)bc/ MAXADC)*5; 
		adc_bv();
		// BasicInfo[1] = ((float)bv/ MAXADC)*5;
		count_sustain();
		//loadcalls();
		mainsystem();
		bat_count();
